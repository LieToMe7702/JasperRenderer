...
vec3 barycentric(const std::vector<vec3>& points, const vec2& p)
{
	vec3 vec_x(points[2][0] - points[0][0], points[1][0] - points[0][0], points[0][0] - p[0]);
	vec3 vec_y(points[2][1] - points[0][1], points[1][1] - points[0][1], points[0][1] - p[1]);
	vec3 cross_vec = cross(vec_x, vec_y);
	if (std::abs(cross_vec.z) < 1e-3) return { -1, 1, 1 };
	vec3 res{ 1.0f - (cross_vec.x + cross_vec.y) / cross_vec.z,cross_vec.y / cross_vec.z,cross_vec.x / cross_vec.z };
	return res;
}
...
void triangle_barycentric(const std::vector<vec3>& points, TGAImage& image, const TGAColor& color, std::vector<double>& z_buffer)
{
	if (points.size() != 3)
	{
		std::cerr << "input point size is not 3" << std::endl;
...
{
	triangle_barycentric({ t0,t1,t2 }, image, color,z_buffer);
}
